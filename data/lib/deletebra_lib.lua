...
function upgradeGuildLevel(cid, guildId)
	...
	if guildId and guildId ~= 0 then
		...
		if level < 10 then
			...
			doPlayerSendTextMessage(cid, MESSAGE_EVENT_ADVANCE, "Parabns!! sua guild subiu de level\n[Level atual]: "..(level+1))
			return true
		else
		...
		doPlayerSendTextMessage(cid, MESSAGE_EVENT_ADVANCE, "Voc no possui guild!")
		return false
	end
...
function giveRewardTable(cid, tableReward, parcelitem)
	...
	for i=1, #tableReward do
		if not isItemStackable(tableReward[i][1]) then	-- CASO //NO// SEJA STACKAVEL
			if tableReward[i][2] > 1 then		-- no stackavel mas  pra add mais que 1
				for j=1, tableReward[i][2] do		-- For s pra add varios items no stackaveis
