	...
	if (p[1]) then
		...
		if (p[1] == "add") then
			...
			if numOffer >= config.offerLimit then
				doPlayerSendTextMessage(cid, config.errorMsgType, "Desculpe, você ja tem "..config.offerLimit.." ofertas ativas.")
				setPlayerStorageValue(cid, 666, os.time()+10)
				return true
			elseif (item.uid <= 0) then
				doPlayerSendTextMessage(cid, config.errorMsgType, "Pra criar alguma oferta, é necessario ter um item no Arrow Slot.")
				setPlayerStorageValue(cid, 666, os.time()+10)
				return true
			elseif isCorpse(item.uid) then
				doPlayerSendTextMessage(cid, config.errorMsgType, "Desculpe, você não pode vender corpse.")
				setPlayerStorageValue(cid, 666, os.time()+10)
				return true
			elseif isInArray(config.blokedItems, item.itemid) then
                doPlayerSendTextMessage(cid, config.errorMsgType, "Desculpe, não pode colocar esse item no TradeOff.")
				setPlayerStorageValue(cid, 666, os.time()+10)
                return true
			end
			...
			if isContainer(item.uid) then
				...
				for j=1, #getBlockedsInContainer do
					if #getBlockedsInContainer[j] > 0 then
						doPlayerSendTextMessage(cid, config.errorMsgType, "Desculpe, você não pode colocar o item "..getItemNameById(config.blokedItems[j]).." no tradeOff.")
						doPlayerSendTextMessage(cid, config.errorMsgType, "Creating log character to ADM...")
						local log = 'Tentou colocar item bloqueado no tradeoff\nName:'..getPlayerName(cid)..'\nAcc: '..getPlayerAccount(cid)..' \n-------------------------------------------------------'
